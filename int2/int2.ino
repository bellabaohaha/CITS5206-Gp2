...
RTC_DATA_ATTR int packet_index = 1;
...
int depth_1 = 10;  // Depth at 10cm
int depth_2 = 20;  // Depth at 20cm
int depth_3 = 30;  // Depth at 30cm
...
const int scheduledTimes[] = { (10 * 60) + 54, (10 * 60) + 56 }; // 17:10  and 17:13 
...
const int PAYLOAD_SIZE = 13;

void createAndPrintPayload() {
  // Initialize an array of Strings to hold each payload element
  String payloadArray[PAYLOAD_SIZE];
  ...
  // Assign date and time to the payload array
  payloadArray[1] = String(current_date);
  payloadArray[2] = String(current_time);

  // Get battery percentage and append '%' symbol
  int battery_percentage = PMU->getBatteryPercent();
  char battery_percentage_str[6]; // e.g., "100%" + null terminator
  snprintf(battery_percentage_str, sizeof(battery_percentage_str), "%d%%", battery_percentage);
  payloadArray[5] = String(battery_percentage_str);

  // Get latitude and longitude separately as strings
  double latitude = lastLatitude;
  double longitude = lastLongitude;

  char latitude_str[16];   // Enough to hold "-123.456789\0"
  char longitude_str[16];  // Enough to hold "123.456789\0"
  snprintf(latitude_str, sizeof(latitude_str), "%.6f", latitude);
  snprintf(longitude_str, sizeof(longitude_str), "%.6f", longitude);

  // Assign latitude and longitude to the payload array
  payloadArray[3] = String(latitude_str);
  payloadArray[4] = String(longitude_str);

  // Assign sensor_board_id to the first element
  payloadArray[0] = String(sensor_board_id);

  // Convert depth and humidity values to strings and assign to the payload array
  payloadArray[6]  = String(depth_1);
  payloadArray[7]  = String(percentageHumidity1);
  payloadArray[8]  = String(depth_2);
  payloadArray[9]  = String(percentageHumidity2);
  payloadArray[10] = String(depth_3);
  payloadArray[11] = String(percentageHumidity3);
  payloadArray[12] = String(packet_index);

 

  // **Concatenate the array elements with commas as delimiters**
  String payload = "";
  for (int i = 0; i < PAYLOAD_SIZE; i++) {
    payload += payloadArray[i];
    if (i < PAYLOAD_SIZE - 1) {
      payload += ","; // Delimiter between elements
    }
  }

  // Print each element of the payload array to Serial Monitor (for debugging)
  Serial.println("Payload Array Elements:");
  for (int i = 0; i < PAYLOAD_SIZE; i++) {
    Serial.print("payloadArray[");
    Serial.print(i);
    Serial.print("]: ");
    Serial.println(payloadArray[i]);
  }

  // Print the concatenated payload to Serial Monitor
  Serial.print("Concatenated Payload: ");
  Serial.println(payload);

  // Initialize and send the payload over LoRa
  initializeLoRa();

  Serial.println("Sending payload over LoRa...");
  LoRa.beginPacket();
  LoRa.print(payload); // Send the concatenated payload string
  LoRa.endPacket();
  Serial.println("Sent payload successfully");
